...
export default function CategoryMultiSelect({
  ...
  const filteredPopoverCats = React.useMemo(() => {
    const q = popoverSearch.trim().toLowerCase();
    if (!q) return popoverCats;
    return popoverCats.filter((c) => {
      const translated = translateCategory(c.display_name, locale).toLowerCase();
      const original = (c.display_name || "").toLowerCase();
      const query = (c.query || "").toLowerCase();
      const id = (c.id || "").toLowerCase();
      return translated.includes(q) || original.includes(q) || query.includes(q) || id.includes(q);
    });
  }, [popoverCats, popoverSearch, locale]);
    ...
    <div ref={containerRef} className={cn("space-y-3", className)}>
      ...
      {topVisible.length > 0 && (
        <div className="relative">
          ...
          {/* Popover with all categories & interactive search box */}
          {popoverOpen && (
            <div className="absolute z-40 left-0 right-0 mt-1 rounded-lg border border-slate-300 bg-white shadow-2xl shadow-black/15 dark:border-white/15 dark:bg-slate-900 dark:shadow-black/60 max-h-80 overflow-y-auto">
              {/* Sticky Header with Title & Search Input */}
              <div className="sticky top-0 z-10 bg-white/95 dark:bg-slate-900/95 backdrop-blur-sm border-b border-slate-200 dark:border-white/10">
                <div className="px-3 py-2 flex items-center justify-between">
                  <span className="text-xs uppercase tracking-wide text-slate-700 dark:text-slate-300 font-bold">
                    {t("radar.allCategories", "Todas")} ({filteredPopoverCats.length})
                  </span>
                  <button
                    type="button"
                    onClick={() => setPopoverOpen(false)}
                    className="text-slate-500 hover:text-slate-900 dark:hover:text-slate-200 text-xs font-semibold"
                  >
                ...
                <div className="px-2.5 pb-2">
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder={t("radar.searchCategoryPlaceholder", "Buscar categoría...")}
                      className="w-full pl-8 pr-7 py-1 text-xs rounded-md border border-slate-300 dark:border-white/10 bg-slate-50 dark:bg-slate-950/60 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-sky-500"
                      autoFocus
                    />
                    {popoverSearch && (
                      <button
              ...
              {popoverLoading ? (
                <div className="px-3 py-6 text-center text-xs text-slate-500">
                  <svg className="inline-block animate-spin text-sky-500 dark:text-sky-400 mr-1" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                    <path d="M21 12a9 9 0 1 1-6.219-8.56" />
                  </svg>
                  Cargando categorías...
                </div>
                ...
                <div className="p-1.5 space-y-0.5">
                  {filteredPopoverCats.map((c) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "w-full text-left px-2.5 py-1.5 flex items-center gap-2 rounded-md text-xs transition-colors",
                          isSelected
                            ? "bg-sky-100 text-sky-900 font-semibold dark:bg-sky-500/25 dark:text-sky-100"
                            : "hover:bg-slate-100 text-slate-800 dark:hover:bg-slate-800/60 dark:text-slate-200"
                        )}
                      >
                        <span aria-hidden="true" className="text-sm">{c.icon || "🏷️"}</span>
                        <span className="flex-1 min-w-0 truncate">{translateCategory(c.display_name, locale)}</span>
                        {c.usage_count > 0 && (
                          <span className="text-amber-600 dark:text-amber-400 text-[10px] font-mono shrink-0">
                            {c.usage_count}×
                          </span>
                        )}
                        {isSelected && (
                  ...
                  {filteredPopoverCats.length === 0 && (
                    <div className="px-3 py-6 text-center text-xs text-slate-500">
                      {popoverSearch
                        ? `Sin resultados para "${popoverSearch}"`
